...
if __name__ == "__main__":
    ...
    for city_j in r_j:
        rc = requests.get(
            "http://0.0.0.0:5000/api/v1/cities/{}/places".format(city_j.get('id')))
        rc_j = rc.json()
    ...
    """
    r = requests.get(
        "http://0.0.0.0:5000/api/v1/cities/{}/places".format(city_id))
    r_j = r.json()
    place_id = r_j[1].get('id')
    ...
    """
    print(place_id)
    r = requests.post("http://0.0.0.0:5000/api/v1/places/{}/reviews/".format(place_id), data=json.dumps(
        {'user_id': user_id, "name": "Amazing Experience", 'text': "NewReview"}), headers={'Content-Type': "application/json"})
